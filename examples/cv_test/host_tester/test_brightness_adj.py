...
def run_kelvin_operator(tester, image_path):
    ...
    brightness_ratio = 50
    params = [0]*10
    params[0] = int(constrast_ratio*256)
    params[1] = brightness_ratio

    # 执行算子 (OP_YU12_TO_NV12 = 3)
    status, out_len, yu12_bytes, cycles = tester.run_kernel(13, img_yuv.tobytes(), w, h, params=params, timeout=10)

    if status == 0:
        # 将结果转回 BGR 验证
        yu12_data = np.frombuffer(yu12_bytes, dtype=np.uint8).reshape(h*3//2, w)
        res_bgr = cv.cvtColor(yu12_data, cv.COLOR_YUV2BGR_I420)
