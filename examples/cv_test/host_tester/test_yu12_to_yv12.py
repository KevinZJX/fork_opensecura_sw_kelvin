...
def run_kelvin_operator(tester, image_path):
    ...
    # 执行算子 (OP_YU12_TO_YV12 = 1)
    status, out_len, out_bytes, cycles = tester.run_kernel(1, img_yuv.tobytes(), w, h, timeout=10)

    if status == 0:
        # 将结果转回 BGR 验证
        yv12_data = np.frombuffer(out_bytes, dtype=np.uint8).reshape(h*3//2, w)
        res_bgr = cv.cvtColor(yv12_data, cv.COLOR_YUV2BGR_YV12)
